...
describe('zoom da interface e texto do terminal', () => {
  ...
  it('valores escolhidos sobrevivem ao reabrir; valor fora do limite no arquivo vira o padrão', () => {
    ...
    saveState(file, { ...emptyState(), settings: { ...emptyState().settings, zoom: 1.3, terminalFontSize: 18 } })
    expect(loadState(file).settings).toMatchObject({ zoom: 1.3, terminalFontSize: 18 })

    writeFileSync(file, JSON.stringify({ version: 2, projects: [], tabs: [], settings: { theme: 'dark', zoom: 9, terminalFontSize: 3 } }))
    expect(loadState(file).settings).toEqual({ theme: 'dark', zoom: 1, terminalFontSize: 14 })
  })
})
